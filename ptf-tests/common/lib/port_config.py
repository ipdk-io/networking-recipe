...
class PortConfig(object):
    ...
    class _GNMICTL(_Common):
        ...
        def gnmi_ctl_get(self, mandatory_params, key):
            ...
            """
            cmd = self.form_cmd(
                f'get "{mandatory_params},{key}" |egrep "*_val" | cut -d ":"  -f 2'
            )
            output, return_code, _ = self.local.execute_command(cmd)
            if return_code:
                log.failed(f"{cmd}")
        ...
        def gnmi_ctl_get_counter(self, mandatory_params, key="counters"):
            ...
            """
            cmd = self.form_cmd(
                f'get "{mandatory_params},{key}" |grep "name\\|uint_val"|grep -v "interface\\|key\\|config\\|counters"'
            )
            output, return_code, _ = self.local.execute_command(cmd)
            if return_code:
                log.failed(f"{cmd}")
    ...
    class _IpCMD(_Common):
        ...
        def __init__(self, remote=False, hostname="", username="", passwd=""):
            ...
            if remote:
                self.connection = Ssh(
                    hostname=hostname, username=username, passwrd=passwd
                )
        ...
        def iplink_add_vlan_port(self, id, name, netdev_port):
            ...
            """
            cmd = self.form_cmd(
                f" link add link {netdev_port} name {name} " f"type vlan id {id}"
            )
            output, return_code, err = self.connection.execute_command(cmd)
            if return_code:
                raise ExecuteCMDException(
                    f'FAIL:command "{cmd}" failed with ' f"an error {err}"
                )
            log.passed(f"{cmd}")
            return True
        ...
        def iplink_create_veth_interface(self, veth_iface, veth_peer_iface):
            ...
            """
            cmd = self.form_cmd(
                f" link add {veth_iface} type veth peer name " f"{veth_peer_iface}"
            )
            output, return_code, err = self.connection.execute_command(cmd)
            if return_code:
                raise ExecuteCMDException(
        ...
        def iplink_add_veth_to_netns(self, namespace_name, veth_iface):
            ...
            :rtype: boolean e.g. True on success, script exits on failure
            """
            cmd = self.form_cmd(f" link set {veth_iface} netns " f"{namespace_name}")
            output, return_code, err = self.connection.execute_command(cmd)
            if return_code:
                raise ExecuteCMDException(
                    f'FAIL:command "{cmd}" failed with ' f"an error {err}"
