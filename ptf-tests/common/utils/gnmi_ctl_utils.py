...
def gnmi_get_params_verify(params):
    ...
    for param in params:
        ...
        for entry in param.split(",")[2:]:
            if (
                port_config.GNMICTL.gnmi_ctl_get(mandatory_param, entry.split(":")[0])
                != entry.split(":")[1]
            ):
                passed = False
    ...
    if [x for x in results if not x]:
        for param in params:
            log.passed(f"gnmi-ctl get verified for {param}")
        return True
...
def gnmi_get_params_elemt_value(params, elemt):
    ...
    for param in params:
        ...
        value = port_config.GNMICTL.gnmi_ctl_get(mandatory_param, elemt).strip()
        if value:
...
def gnmi_get_params_counter(param):
    ...
    value = port_config.GNMICTL.gnmi_ctl_get_counter(
        mandatory_param, "counters"
    ).strip()
...
def ip_add_addr(interface, ip, remote=False, hostname="", username="", passwd=""):
    port_config = PortConfig(
        remote=remote, hostname=hostname, username=username, passwd=passwd
    )
...
def gnmi_get_element_value(param, element):
    ...
    """
    port_config = PortConfig()
    result = port_config.GNMICTL.gnmi_ctl_get(param, element)
    port_config.GNMICTL.tear_down()
...
def get_port_mtu_linuxcli(port):
    ...
    """
    local = Local()
    mtu_value, returncode, err = local.execute_command(
        f"cat /sys/class/net/" + port + "/mtu"
    )
    if returncode:
        log.failed(f"Failed to get MTU for " + port + " port")
        return False
    else:
        return mtu_value
...
def iplink_add_vlan_port(id, name, netdev_port):
    ...
    port_config.GNMICTL.tear_down()
    if result:
        log.passed(f"succeed to add {name} to port {netdev_port}")
        return True
    else:
        log.failed(f"fail to add {name} to port {netdev_port}")
        return False


def ip_set_dev_up(
    devname, status_to_change="up", remote=False, hostname="", username="", password=""
):
    """
     Enable <devname> up
    ...
    port_config.Ip.tear_down()
    log.passed(f"succeed to enable {devname} {status_to_change}")
    return True
...
def iplink_del_port(port_to_delete, remote=False, hostname="", username="", passwd=""):
    ...
    port_config.Ip.tear_down()
    if result:
        log.passed(f"succeed to delee {port_to_delete}")
        return True
    else:
        log.failed(f"fail to delete {port_to_delete}")
        return False
...
def ip_link_add_veth_and_peer(
    veth_iface, veth_peer_iface, remote=False, hostname="", username="", password=""
):
    port_config = PortConfig(
        remote=remote, hostname=hostname, username=username, passwd=password
    )
...
def ip_link_set_veth_to_ns(
    namespace_name, veth_iface, remote=False, hostname="", username="", password=""
):
    port_config = PortConfig(
        remote=remote, hostname=hostname, username=username, passwd=password
    )
...
def ip_link_netns_exec(
    namespace, command, remote=False, hostname="", username="", password=""
):
    port_config = PortConfig(
        remote=remote, hostname=hostname, username=username, passwd=password
    )
...
def iproute_add(
    ...
    if result:
        ...
        log.failed(f"fail to add route")
        return False
...
def iproute_del(dst, remote=False, hostname="", username="", password=""):
    ...
    if result:
        ...
        log.failed(f"fail to delete route")
        return False
...
def iplink_add_dev(name, type, remote=False, hostname="", username="", password=""):
    ...
    if result:
        ...
        log.failed(f"fail to add device {name} type {type}")
        return False
