...
def ovs_p4ctl_add_member_and_verify(bridge, tbl_name, member_details):
    ...
    try:
        ...
        if out == None:
            print(
                f"PASS: ovs-p4ctl get details for member {mem_id} :  {member_returned}"
            )
        ...
        if (
            ...
            print(
                "FAIL : Returned member details are not matching with configured group"
            )
            return False
    except Exception as error:
        print(f"FAIL: ovs-p4ctl add member Failed with error: {error}")
...
def ovs_p4ctl_get_member(bridge, tbl_name, member_id, member_details=None):
    ...
    try:
        ...
        if out == None:
            print(
                f"PASS: ovs-p4ctl get details for member {member_id} :  {member_returned}"
            )

        if member_details != None:
            action, action_data, mem_id = ovs_p4ctl.parse_profile_mem(
...
def ovs_p4ctl_add_group_and_verify(bridge, tbl_name, group_details):
    ...
    try:
        ...
        if out == None:
            print(
                f"PASS: ovs-p4ctl get details for group {group_id} :  {group_returned}"
            )
        if (
            ...
            print(
                "FAIL : Returned group details are not matching with configured group"
            )
            return False
...
def ovs_p4ctl_get_group(bridge, tbl_name, group_id, group_details=None):
    ...
    try:
        ...
        if group_details != None:
            group_id, members, max_size = ovs_p4ctl.parse_profile_group(
