...
class SimpleL3Test(BaseTest):
    ...
    def runTest(self):
        try:
            ...
            # tcpdump
            p1 = subprocess.Popen(
                ["tcpdump", "-w", "/tmp/capture1.pcap", "-i", "TAP0"],
            ...
            # tcpdump
            p2 = subprocess.Popen(
                ["tcpdump", "-w", "/tmp/capture2.pcap", "-i", "TAP1"],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
            time.sleep(1)
            ...
            ovs_p4ctl.p4ctl_del_entry(
                "br0", "ingress.ipv4_host", "hdr.ipv4.dst_addr=1.1.1.2"
            )
            ...
            # tcpdump
            p3 = subprocess.Popen(
                ["tcpdump", "-w", "/tmp/capture3.pcap", "-i", "TAP1"],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
            time.sleep(1)
