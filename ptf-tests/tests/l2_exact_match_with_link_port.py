        ...
        for table in self.config_data["table"]:
            ...
            if table["description"] == "table_for_dst_mac":
                pkt = simple_tcp_packet(
                    eth_dst=self.config_data["traffic"]["in_pkt_header"]["eth_mac_1"]
                )
            else:
                pkt = simple_tcp_packet(
                    eth_src=self.config_data["traffic"]["in_pkt_header"]["eth_mac_1"]
                )
            ...
            try:
                ...
                log.passed(
                    "PASS: Verification of packets passed, packet received as per rule 1"
                )
            except Exception as err:
                self.result.addFailure(self, sys.exc_info())
                self.fail(
                    f"FAIL: Verification of rule 1 packets sent failed with exception {err}"
                )

            # forming 2nd packet and sending to validate if rule-2 hits or not
            log.info("sending packet to check if rule 2  hits")
            if table["description"] == "table_for_dst_mac":
                pkt = simple_tcp_packet(
                    eth_dst=self.config_data["traffic"]["in_pkt_header"]["eth_mac_2"]
                )
            else:
                pkt = simple_tcp_packet(
                    eth_src=self.config_data["traffic"]["in_pkt_header"]["eth_mac_2"]
                )
            ...
            try:
                ...
                log.passed(
                    "PASS: Verification of packets passed, packet received as per rule 2"
                )
            except Exception as err:
                self.result.addFailure(self, sys.exc_info())
                self.fail(
                    f"FAIL: Verification of rule 2 packets sent failed with exception {err}"
                )

            # forming 2th packet and sending to validate if rule-3 hits or not
            log.info("sending packet to check if rule 3 hits")
            if table["description"] == "table_for_dst_mac":
                pkt = simple_tcp_packet(
                    eth_dst=self.config_data["traffic"]["in_pkt_header"]["eth_mac_3"]
                )
            else:
                pkt = simple_tcp_packet(
                    eth_src=self.config_data["traffic"]["in_pkt_header"]["eth_mac_3"]
                )
            ...
            try:
                ...
                log.passed(
                    "PASS: Verification of packets passed, packet dropped as per rule 3"
                )
            except Exception as err:
                self.result.addFailure(self, sys.exc_info())
                self.fail(
                    f"FAIL: Verification of rule 3 packets sent failed with exception {err}"
                )

            # forming 2th packet and sending to validate if rule-4 hits or not
            log.info("sending packet to check if rule 4 hits")
            if table["description"] == "table_for_dst_mac":
                pkt = simple_tcp_packet(
                    eth_dst=self.config_data["traffic"]["in_pkt_header"]["eth_mac_4"]
                )
            else:
                pkt = simple_tcp_packet(
                    eth_src=self.config_data["traffic"]["in_pkt_header"]["eth_mac_4"]
                )
            ...
            try:
                verify_no_packet_any(
                    ...
                    pkt,
                    device_number=0,
                    ports=[port_ids[self.config_data["traffic"]["receive_port"][1]][0]],
                )
                log.passed(
                ...
                self.result.addFailure(self, sys.exc_info())
                self.fail(
                    f"FAIL: Verification of rule 4 packets sent failed with exception {err}"
                )
