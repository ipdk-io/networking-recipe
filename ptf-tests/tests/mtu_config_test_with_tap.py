...
from common.utils.tcpdump_utils import (
    tcpdump_start_pcap,
    tcpdump_get_pcap,
    tcdump_match_str,
    tcpdump_tear_down,
...
class TapPort_MTU_Config(BaseTest):
    ...
    def runTest(self):
        ...
        for table in self.config_data["table"]:
            # Add rules for p4 programm
            log.info(f"Scenario : Verifying MTU configuration on DPDK Tap Ports")
            log.info(f"Adding rules")
            for match_action in table["match_action"]:
                p4rt_ctl.p4rt_ctl_add_entry(
                    table["switch"], table["name"], match_action
                )

        # Set Mtu to the ports
        ...
        # sending packet from TAP0 of size 1400  i.e. greater than assigned MTU of 500
        log.info(
            f"sending packet from "
            + port_list[0]
            + " of size "
            + pktsize1
            + "  i.e. greater than assigned MTU of "
            + assigned_mtu
        )
        ...
        try:
            send_packet(
                self, port_ids[self.config_data["traffic"]["send_port"][0]][1], pkt
            )
            ...
            if result:
                log.passed(
                    f"Assertion verified as per difference in MTU and packet size"
                )
            else:
                self.result.addFailure(self, sys.exc_info())
                self.fail(
                    f"FAIL: Assertion not verified as per difference in MTU and packet size"
                )
        else:
            try:
                time.sleep(1)
        ...
        # sending packet from TAP1 of size 1400
        pkt = simple_tcp_packet(
            eth_src=src_mac,
            eth_dst=dst_mac,
            ip_src=ip_src,
        ...
        log.info("sending packet from " + port_list[1] + " of size " + pktsize1)
        ...
        try:
            ...
            log.passed(f"Verification of packets passed, packet received as expected")
        except Exception as err:
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAIL: Verification of packets sent failed with exception {err}")

        # sending packet from TAP0 of size 300
        pkt = simple_tcp_packet(
            eth_src=src_mac,
            eth_dst=dst_mac,
            ip_src=ip_src,
        ...
        log.info("sending packet from " + port_list[0] + " of size " + pktsize2)
        try:
            verify_packet(self, pkt, port_id=port_ids[1][1], timeout=None)
            log.passed(f"Verification of packets passed, packet received as expected")
        except Exception as err:
        ...
        # sending packet from TAP1 of size 300
        pkt = simple_tcp_packet(
            eth_src=src_mac,
            eth_dst=dst_mac,
            ip_src=ip_src,
        ...
        send_packet(self, port_ids[self.config_data["traffic"]["send_port"][1]], pkt)
        log.info("sending packet from " + port_list[1] + " of size " + pktsize2)
        try:
