...
class L3_Exact_Match(BaseTest):
    ...
    def runTest(self):
        ...
        try:
            # Verify no any traffic packet received
        ...
        send_packet(self, port_ids[self.config_data["traffic"]["send_port"][1]], pkt)
        try:
            # Verify traffic packet received
            verify_packet(
                self, pkt, port_ids[self.config_data["traffic"]["receive_port"][1]][1]
            )
            log.info(f"Verification of packets passed, packet received as per rule 2")
        except Exception as err:
            self.result.addFailure(self, sys.exc_info())
            log.info(f"Verification of packets sent failed with exception {err}")
