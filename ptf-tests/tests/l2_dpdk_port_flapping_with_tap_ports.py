...
class DpdkPortFlapping(BaseTest):
    ...
    def setUp(self):
        ...
        5. Prepare list of gnmi_ctl cmds needed to create vHost ports
        ...
        config_json = test_params[
            "config_json"
        ]  # "--test_params" parameter passed from cmd line
        self.config_data = config_file_utils.get_config_dict(config_json)
        # Prepare list of gnmi_ctl cmds needed to create vHost ports
        self.gnmictl_params_list = config_file_utils.get_gnmi_params_simple(
            self.config_data
        )
    ...
    def runTest(self):
        ...
        2. set TAP interfaces using gnmi_ctl set and bring up interfaces
        ...
        # set TAP interfaces using gnmictl set
        if not gnmi_ctl_utils.gnmi_ctl_set_and_verify(self.gnmictl_params_list):
            self.result.addFailure(self, sys.exc_info())
            self.fail("Failed to configure gnmi ctl ports")
        # Assign IP to TAP interfaces and bring up
        gnmi_ctl_utils.ip_set_ipv4(self.interface_ip_list)
        ...
        port_ids = test_utils.add_port_to_dataplane(self.config_data["port_list"])
        ...
        for table in self.config_data["table"]:
            log.info(
                f"Scenario : l2 dpdk port flapping with tap ports: "
                f"{table['description']}"
            )
            ...
            try:
                ptf.testutils.verify_packet(
                    self, pkt, port_ids[self.config_data["traffic"]["receive_port"][0]]
                )
                log.passed(
                    f"Verification of packet passed, packet received " f"as per rule 1"
                )
            except Exception as e:
                self.fail(
                    f"FAIL: Verification of packet sent failed with " f"exception {e}"
                )
            ...
            try:
                ptf.testutils.verify_no_packet(
                    self, pkt, port_ids[self.config_data["traffic"]["receive_port"][1]]
                )
                log.passed(
                    f"Verification of packet passed, packet dropped " f"as per rule 2"
                )
            except Exception as e:
                self.fail(
                    f"FAIL: Verification of packet sent failed with " f"exception {e}"
        ...
        log.info("Bring TAP interfaces down...")
        ...
        for interface_ipv4_dict in self.interface_ip_list:
            for interface, ip in interface_ipv4_dict.items():
                portconfig_obj.Ip.iplink_enable_disable_link(
                    interface, status_to_change="down"
                )
        log.info("TAP interfaces are down!!!")
        time.sleep(5)

        # Now resend packets to verify that traffic should not be forwarded
        log.info("Resend traffic to verify traffic is not getting forwarded...")
        pkt_counter = 0
        for table in self.config_data["table"]:
            ...
            try:
                ...
                ptf.testutils.verify_no_packet(
                    self,
                    tcp_packet_list[pkt_counter],
                    port_ids[self.config_data["traffic"]["receive_port"][0]],
                )
                log.passed(
                    f"Verification of packet passed, packet didn't "
                    f"received as port is down"
                )
            except OSError as OE:
                if "Network is down" in str(OE):
                    log.passed(f"Traffic send failed as Ports are down.")
            except Exception as e:
                self.fail(
                    f"FAIL: Verification of packet sent failed with " f"exception {e}"
                )
            ...
            try:
                ...
                ptf.testutils.verify_no_packet(
                    self,
                    tcp_packet_list[pkt_counter],
                    port_ids[self.config_data["traffic"]["receive_port"][1]],
                )
                log.passed(
                    f"Verification of packet passed, packet didn't "
                    f"received as port is down"
        ...
        log.info(f"Bringing TAP interfaces up...")
        for interface_ipv4_dict in self.interface_ip_list:
            for interface, ip in interface_ipv4_dict.items():
                portconfig_obj.Ip.iplink_enable_disable_link(
                    interface, status_to_change="up"
                )
        ...
        time.sleep(5)
        log.info(f"TAP interfaces are up.")

        # Now resend packets to verify that traffic should be sent
        log.info("Resend traffic to verify traffic is getting forwarded...")
        pkt_counter = 0
        for table in self.config_data["table"]:
            log.info(port_ids[self.config_data["traffic"]["send_port"][0]])
            # Send first packet to TAP1 port
            ptf.testutils.send_packet(
                self,
                port_ids[self.config_data["traffic"]["send_port"][0]],
            ...
            try:
                ...
                log.passed(
                    f"Verification of packet passed, packet received " f"as per rule1"
                )
            except Exception as e:
                self.fail(
                    f"FAIL: Verification of packet sent failed with " f"exception {e}"
            ...
            try:
                ...
                log.passed(
                    f"Verification of packet passed, packet dropped as " f"per rule1"
                )
            except Exception as e:
                self.fail(
                    f"FAIL: Verification of packet sent failed with " f"exception {e}"
    ...
    def tearDown(self):
        """It removes tdi pipeline table entries, rules
        :return: None
        :rtype: None
        """
        for table in self.config_data["table"]:
