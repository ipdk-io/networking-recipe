...
class L3_Exact_Match(BaseTest):
    ...
    def runTest(self):
        ...
        if not result:
            self.result.addFailure(self, sys.exc_info())
            log.failed("Ping test failed from VM0")
        ...
        if not result:
            self.result.addFailure(self, sys.exc_info())
            log.failed("Ping test failed from VM1")

        # delete l3 exact match forwarding rule
        log.info("Delete Rule 2")
        if not p4rt_ctl.p4rt_ctl_del_entry(
            self.config_data["table"][0]["switch"],
            self.config_data["table"][0]["name"],
            self.config_data["table"][0]["del_action"][1],
        ):
            self.result.addFailure(self, sys.exc_info())
            self.fail(
                f"Failed to delete table entry {self.config_data['table'][0]['del_action'][0]}"
            )

        # ping test between VMs
        log.info("Ping test from VM0 to VM1 : Drop check")
        ...
        if not result:
            self.result.addFailure(self, sys.exc_info())
            log.failed("Ping drop test failed from VM0")

        log.info("Ping test from VM1 to VM0 : Drop check")
        result = test_utils.vm_to_vm_ping_drop_test(
            conn2, self.config_data["vm"][1]["remote_ip"]
        )
        if not result:
            self.result.addFailure(self, sys.exc_info())
            log.failed("Ping drop test failed from VM1")

        # Add Rule 2 back
        log.info("Add Rule 2 Back")
        if not p4rt_ctl.p4rt_ctl_add_entry(
            self.config_data["table"][0]["switch"],
            self.config_data["table"][0]["name"],
            self.config_data["table"][0]["match_action"][1],
        ):
            self.result.addFailure(self, sys.exc_info())
            self.fail(
                f"Failed to add table entry {self.config_data['table'][0]['match_action'][1]}"
            )
