...
class LNT_ECMP_DEL_ADD_RULE(BaseTest):
    ...
    def runTest(self):
        ...
        for tap_port in self.tap_port_list:
            if not gnmi_ctl_utils.ip_set_dev_up(tap_port):
                self.result.addFailure(self, sys.exc_info())
        ...
        for i in range(len(self.conn_obj_list)):
            ...
            if not gnmi_ctl_utils.iplink_add_vlan_port(
                id, vlanname, self.tap_port_list[1]
            ):
                self.result.addFailure(self, sys.exc_info())
        ...
        # overlay ping
        log.info(f"Ping test executed from VM on local host")
        for i in range(len(self.conn_obj_list)):
        ...
        log.info("Start rule delete and re-add test")
        i = 0
        for table in self.config_data["table"]:
            for del_action in table["del_action"]:
                log.info(f"Delete rule {del_action}")
            ...
            if not test_utils.vm_to_vm_ping_drop_test(
                ...
                self.result.addFailure(self, sys.exc_info())
                self.fail(
                    f"FAIL: Ping expected to fail but succeed after deleting rule"
                )

            for match_action in table["match_action"]:
                log.info(f"add back rule {match_action}")
                if not p4rt_ctl.p4rt_ctl_add_entry(
                    table["switch"], table["name"], match_action
                ):
                    self.result.addFailure(self, sys.exc_info())
