...
class DpdkPortFlappingVhost(BaseTest):
    ...
    def setUp(self):
        ...
        self.config_data = config_file_utils.get_config_dict(
            config_json, "", vm_location_list, self.vm_cred
        )
    ...
    def runTest(self):
        ...
        2.set vHost interfaces using gnmi_ctl set
        ...
        # set vHost interfaces using gnmi_ctl set
        ...
        for vm, port in zip(self.config_data["vm"], self.config_data["port"]):
            globals()["conn" + str(vm_id + 1)] = telnet_connection.connectionManager(
        ...
        for table in self.config_data["table"]:
            log.info(
                f"Scenario : l3 dpdk port flapping with vHost ports: "
                f"{table['description']}"
            )
        ...
        log.info(f"Ping test from VM1 to VM2...")
        result = test_utils.vm_to_vm_ping_test(
            conn1, self.config_data["vm"][0]["remote_ip"]
        )
        if not result:
            self.result.addFailure(self, sys.exc_info())
            log.failed("FAIL: Ping test failed for VM1")
        # Port flapping test
        log.info("Test VM2 port flapping while traffic from VM1 -> VM2 running")
        result = test_utils.vm_port_flapping(conn1, self.config_data, self.result)
        if not result:
            self.result.addFailure(self, sys.exc_info())
            log.failed("FAIL: L3 DPDK port flapping with vHost port failed")
