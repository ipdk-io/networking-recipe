...
class LNT_4VM_FUll_MESH(BaseTest):
    ...
    def runTest(self):
        ...
        # Configure vxlan on remote host
        if not ovs_utils.add_vxlan_port_to_ovs(
            self.config_data["bridge"],
            self.config_data["vxlan"]["vxlan_name"][0],
            self.config_data["vxlan"]["tep_ip"][1].split("/")[0],
        ...
        # Ping test executed from VM on remote host
        for namespace in self.config_data["net_namespace"]:
            for ip in namespace["remote_ping"]:
                test_utils.ip_ntns_exec_ping_test(
                    namespace["name"],
                    ip,
                    remote=True,
                    hostname=self.config_data["client_hostname"],
        ...
        # Ping test executed from VM on local host
    ...
    def tearDown(self):
        ...
        # Delete p4ovs match action rules on local host
        for table in self.config_data["table"]:
