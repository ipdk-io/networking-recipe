...
"""
1 VM on local host with OVS and 1 NS VM on remote host with Stand OVS
"""
...
class LNT_1HotplugVM_Vxlan_1NsVM(BaseTest):
    ...
    def runTest(self):
        ...
        # ----------------------------------#
        # Give time for network up
        time.sleep(10)
        log.info("Ping test executed from VM on remote host")
        for namespace in self.config_data["net_namespace"]:
            for ip in namespace["remote_ping"]:
                # Execute ping rom remote host
