...
"""
LNT FRR with ECMP modify BGP ASN on 2 Hosts having 4 VMs
"""
...
class LNT_FRR_with_ECMP_modify_BGP_ASN(BaseTest):
    def setUp(self):
        BaseTest.setUp(self)
    ...
    def runTest(self):
        ...
        if not gnmi_ctl_utils.gnmi_ctl_set_and_verify(self.gnmicli_params):
            self.result.addFailure(self, sys.exc_info())
            self.fail("FAIL: failed to configure gnmi ctl ports")
        ...
        if not test_utils.run_frr_service("start"):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAIL: failed to start frr service on local host")
        if not test_utils.run_frr_service(
            "start",
            remote=True,
            hostname=self.config_data["client_hostname"],
            ...
            self.result.addFailure(self, sys.exc_info())
            self.fail(
                f"FAIL: failed to start frr service on {self.config_data['client_hostname']}"
            )
        ...
        if not result:
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAIL: VM creation failed for {vm_name}")
        ...
        if not test_utils.gen_dep_files_p4c_dpdk_pna_tdi_pipeline_builder(
            ...
            self.result.addFailure(self, sys.exc_info())
            self.fail("FAIL: failed to generate P4C artifacts or pb.bin")
        ...
        if not p4rt_ctl.p4rt_ctl_set_pipe(
            ...
            self.result.addFailure(self, sys.exc_info())
            self.fail("FAIL: Failed to set pipe")

        # Bring up TAP ports
        if not gnmi_ctl_utils.ip_set_dev_up(self.tap_port_list[0]):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAIL: Failed to bring up {self.tap_port_list[0]}")
        ...
        if not gnmi_ctl_utils.iplink_add_dev(
            ...
            self.result.addFailure(self, sys.exc_info())
            self.fail(
                f"FAIL: Failed to add dev {self.config_data['vxlan']['tep_intf']} type dummy"
            )
        ...
        for i in range(len(self.config_data["ecmp"]["local_ports"])):
            if not test_utils.vtysh_config_frr_router_interface(
                ...
                self.result.addFailure(self, sys.exc_info())
                self.fail(
                    f"FAIL: failed to config frr inteface {self.config_data['ecmp']['local_ports_ip'][i]} on local"
                )
        ...
        if not ovs_utils.add_bridge_to_ovs(self.config_data["bridge"]):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAIL: failed to add bridge {self.config_data['bridge']} to ovs")
        ...
        if not ovs_utils.add_vxlan_port_to_ovs(
            ...
            self.result.addFailure(self, sys.exc_info())
            self.fail(
                f"FAIL: failed to add vxlan {self.config_data['vxlan']['vxlan_name'][0]} to bridge {self.config_data['bridge']}"
            )
        # Configure VLAN
        for i in range(len(self.conn_obj_list)):
            ...
            if not gnmi_ctl_utils.iplink_add_vlan_port(
                ...
                self.result.addFailure(self, sys.exc_info())
                self.fail(
                    f"FAIL: Failed to add vlan {vlanname} to {self.tap_port_list[0]}"
                )
            ...
            if not ovs_utils.add_vlan_to_bridge(self.config_data["bridge"], vlanname):
                self.result.addFailure(self, sys.exc_info())
                self.fail(
                    f"FAIL: Failed to add vlan {vlanname} to {self.config_data['bridge']}"
                )

            # Bring up vlan
            if not gnmi_ctl_utils.ip_set_dev_up(vlanname):
                self.result.addFailure(self, sys.exc_info())
                self.fail(f"FAIL: Failed to bring up {vlanname}")
        ...
        for table in self.config_data["table"]:
            ...
            for match_action in table["match_action"]:
                if not p4rt_ctl.p4rt_ctl_add_entry(
                    ...
                    self.result.addFailure(self, sys.exc_info())
                    self.fail(f"FAIL: Failed to add table entry {match_action}")
        ...
        # cConfigure BGP attributes on remote frr
        for i in range(len(self.config_data["ecmp"]["local_ports_ip"])):
            if not test_utils.vtysh_config_frr_bgp_attr(
                ...
                self.result.addFailure(self, sys.exc_info())
                self.fail(
                    f"FAIL: Failed to config Bgp on {self.config_data['client_hostname']}"
                )
        ...
        time.sleep(5)
        log.info(
            f"Ping test for underlay network before modifying ASN-{self.config_data['bgp']['as_number'][0]}"
        )
        for ip in self.config_data["ecmp"]["remote_ports_ip"]:
            ping_cmd = f"ping {ip.split('/')[0]} -c 10"
            # Execute remote port ping
        ...
        log.info(
            f"Verify overlay ping before modifying ASN-{self.config_data['bgp']['as_number'][0]}"
        )
        for i in range(len(self.conn_obj_list)):
        ...
        # Modify BGP ASN on local host
        log.info(
            f"Begin to modify BGP ASN to {self.config_data['bgp']['as_number'][1]} on localhost"
        )
        ...
        # Add new route on local host
        log.info(f"add new route table on localhost")
        for i in range(len(self.config_data["ecmp"]["remote_ports_ip"])):
            # Configure bgp attribute
            if not test_utils.vtysh_config_frr_bgp_attr(
                self.config_data["bgp"]["as_number"][1],
                neighbor=self.config_data["ecmp"]["remote_ports_ip"][i].split("/")[0],
                addr_family=self.config_data["bgp"]["addr_family"],
                network=self.config_data["vxlan"]["tep_ip"][0],
                router_id=self.config_data["vxlan"]["tep_ip"][0].split("/")[0],
        ...
        # Modify BGP ASN on remote host
        log.info(
            f"Start to modify BGP ASN on remote {self.config_data['client_hostname']}"
        )
        log.info(
        ...
        # Add new bgp ASN
        log.info(f"add new bgp ASN {self.config_data['bgp']['as_number'][1]} on remote")
        for i in range(len(self.config_data["ecmp"]["local_ports_ip"])):
            # Configure bgp attribute
            if not test_utils.vtysh_config_frr_bgp_attr(
                self.config_data["bgp"]["as_number"][1],
        ...
        time.sleep(5)
        log.info(
            f"Ping test for underlay with new ASN {self.config_data['bgp']['as_number'][1]}"
        )
        # ping remote tep
        for ip in self.config_data["ecmp"]["remote_ports_ip"]:
            ping_cmd = f"ping {ip.split('/')[0]} -c 10"
        ...
        # Execute overlay ping with new ASN
        log.info(
            f"Start overlay ping with new ASN {self.config_data['bgp']['as_number'][1]}"
        )
        for i in range(len(self.conn_obj_list)):
            for ip in self.config_data["vm"][i]["remote_ip"]:
                ...
                while n <= max:
                    ...
                    for send_port_id in self.config_data["traffic"]["send_port"]:
                        send_cont = gnmi_ctl_utils.gnmi_get_params_counter(
                            self.gnmicli_phy_ctrl_params[send_port_id]
                        )
                        if not send_cont:
                            log.failed(
                        ...
                        log.passed(
                            f"The port {self.gnmicli_phy_ctrl_params[send_port_id].split(',')[1].split(':')[1]} "
                            + "counter is built successfully"
                        )
                        send_count_list_before.append(send_cont)

                    # Execute VM to VM ping
                ...
                if stat_total >= self.config_data["traffic"]["number_pkts"][0]:
                    log.passed(
                        f"Minimum {self.config_data['traffic']['number_pkts'][0]} tranmitted and {stat_total} recorded on the path"
                    )
                else:
                    log.failed(
                        f"Minimum {self.config_data['traffic']['number_pkts'][0]} packets expected but only {stat_total} recorded"
                    )
                    self.result.addFailure(self, sys.exc_info())
