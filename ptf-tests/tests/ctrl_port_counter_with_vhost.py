...
class Control_Port_Counter(BaseTest):
    ...
    def runTest(self):
        ...
        log.info(f"Adding control to ovs bridge")
        num = self.config_data["traffic"]["number_pkts"][0]
        result = test_utils.ovs_add_ctrl_port_to_bridge(
            self.config_data["dump_bridge"],
            self.control_port,
            self.config_data["p4_device_id"],
        )
        if result:
            log.passed(f"PASS: control port are added in ovs")
        else:
            self.result.addFailure(self, sys.exc_info())
        ...
        log.info("Ping test from VM0 to control port TAP0")
        ...
        counter1 = test_utils.get_ovs_port_dump(
            self.config_data["dump_bridge"], self.control_port
        )
        result = test_utils.vm_to_vm_ping_drop_test(
            conn1, self.config_data["traffic"]["ip_dst"][0], count=num
        )
        if result:
            log.passed("PASS: expected ping failure BUT at lease it trigger counter")
        counter2 = test_utils.get_ovs_port_dump(
            self.config_data["dump_bridge"], self.control_port
        )

        pkt_count = (
            counter2[self.control_port[0]]["rx"]["pkts"]
            - counter1[self.control_port[0]]["rx"]["pkts"]
        )
        if pkt_count not in range(vm_ping_num, num_buffer):
            self.result.addFailure(self, sys.exc_info())
            self.fail(
                f"FAILED: expected {vm_ping_num} rx pkts counter but {pkt_count} "
            )
        else:
            log.passed(f"PASS: rx pkts counter increased by {vm_ping_num}")

        bytes_count = (
            counter2[self.control_port[0]]["rx"]["bytes"]
            - counter1[self.control_port[0]]["rx"]["bytes"]
        )
        if bytes_count not in range(bytes, bytes_buffer):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAILED: expected {bytes} rx bytes counter but {bytes_count}")
        else:
            log.passed(f"PASS: rx bytes counter increased by {bytes_count}")

        log.info("Sending traffic from control TAP to vhost ")
        ...
        pkt_count = (
            counter2[self.control_port[0]]["tx"]["pkts"]
            - counter1[self.control_port[0]]["tx"]["pkts"]
        )
        if pkt_count not in range(num, num_buffer):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"FAILED: expected {num} tx pkts counter but {pkt_count} ")
        else:
            log.passed(f"PASS: tx pkts counter increased by {num}")

        bytes_count = (
            counter2[self.control_port[0]]["tx"]["bytes"]
            - counter1[self.control_port[0]]["tx"]["bytes"]
        )
        if bytes_count not in range(bytes, bytes_buffer):
            self.result.addFailure(self, sys.exc_info())
