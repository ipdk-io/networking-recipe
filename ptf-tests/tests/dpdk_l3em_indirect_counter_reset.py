...
class IPDK_L3EM_INDIRECT_COUNTER_RESET(BaseTest):
    ...
    def runTest(self):
        ...
        if not test_utils.vm_to_vm_ping_test(
            ...
            log.passed(
                f"The number of {2 * self.config_data['counter']['num_ping'][0]} of ping are completed"
            )
        ...
        if not cnt_table_name_and_id:
            self.result.addFailure(self, sys.exc_info())
            self.fail("FAIL: failed to get counter table name and id")
        for i in range(len(cnt_table_name_and_id)):
            ...
            for j in self.config_data["counter"]["index"]:
                ...
                if not data:
                    self.result.addFailure(self, sys.exc_info())
                    self.fail("FAIL: failed to get counter data")
                cnt1[cnt_table_name].append(data)
        log.info(f"The counter before reset is {cnt1}")

        log.info("Start to reset counter")
        for i in range(len(cnt_table_name_and_id)):
            counter_id = cnt_table_name_and_id[i][1]
            cnt_table_name = cnt_table_name_and_id[i][0]
            cnt2[cnt_table_name] = []
            for j in range(len(self.config_data["counter"]["index"])):
                ...
                log.info(f"Execute reset {flow} of {cnt_table_name}")
                p4rt_ctl.p4rt_ctl_reset_counter_entry(
                    self.config_data["switch"], cnt_table_name, flow
                )
                data = p4rt_ctl.p4rt_ctl_get_counter_data(
                    self.config_data["switch"], cnt_table_name, flow
                ...
                if cnt_table_name == "ipv4_host_tbl_flow_counter_bytes":
                    if cnt1[cnt_table_name][j]["bytes"] <= 0:
                        self.result.addFailure(self, sys.exc_info())
                        self.fail(
                            f"FAIL: The bytes counter of {cnt_table_name} before reset should be greater than 0"
                        )
                    else:
                        log.passed(
                            f"PASS: The byte counter of {cnt_table_name} before reset is {cnt1[cnt_table_name][i]['bytes']}"
                        )
                    log.info(
                ...
                if cnt_table_name == "ipv4_host_tbl_flow_counter_packets":
                    ...
                    if cnt2[cnt_table_name][j]["packets"] > 0:
                        ...
                        log.passed(f"PASS: reset {flow} packet counter to 0 ")
