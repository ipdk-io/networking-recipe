...
class Connection_Track(BaseTest):
    ...
    def runTest(self):
        ...
        # Verification of data traffic before connection establishment
        log.info(
            "Verification of data traffic before connection establishment for TAP0 and Link Port"
        )
        log.info(
            "-----------------------------------------------------------------------------------"
        )
        ...
        pkt = simple_tcp_packet(
            ...
            tcp_sport=self.config_data["traffic"]["in_pkt_header"]["tcp_port"][1],
            tcp_dport=self.config_data["traffic"]["in_pkt_header"]["tcp_port"][0],
            tcp_flags="A",
        )
        ...
        try:
            ...
            log.passed(
                f" Verification of date packet check passed : No traffic between B->A"
            )
        except Exception as err:
            self.result.addFailure(self, sys.exc_info())
            log.failed(
                f" Verification of data packet sent failed with exception {err}: B->A"
            )

        # Verification of data traffic before connection establishment for TAP1 and TAP2 Port
        log.info(
            "Verification of data traffic before connection establishment for TAP1 and TAP2 Port"
        )
        log.info(
        ...
        pkt = simple_tcp_packet(
            eth_src=self.config_data["traffic"]["in_pkt_header"]["eth_mac"][2],
            eth_dst=self.config_data["traffic"]["in_pkt_header"]["eth_mac"][3],
            ip_src=self.config_data["traffic"]["in_pkt_header"]["ip_address"][2],
            ip_dst=self.config_data["traffic"]["in_pkt_header"]["ip_address"][3],
            tcp_sport=self.config_data["traffic"]["in_pkt_header"]["tcp_port"][2],
            tcp_dport=self.config_data["traffic"]["in_pkt_header"]["tcp_port"][3],
            tcp_flags="A",
        )
        ...
        log.info("Sending data packet: B->A")
        pkt = simple_tcp_packet(
            eth_src=self.config_data["traffic"]["in_pkt_header"]["eth_mac"][3],
            eth_dst=self.config_data["traffic"]["in_pkt_header"]["eth_mac"][2],
            ip_src=self.config_data["traffic"]["in_pkt_header"]["ip_address"][3],
            ip_dst=self.config_data["traffic"]["in_pkt_header"]["ip_address"][2],
            tcp_sport=self.config_data["traffic"]["in_pkt_header"]["tcp_port"][3],
            tcp_dport=self.config_data["traffic"]["in_pkt_header"]["tcp_port"][2],
            tcp_flags="A",
        )
        send_packet(self, port_ids[self.config_data["traffic"]["send_port"][3]], pkt)
        try:
            verify_no_packet(
                self, pkt, port_ids[self.config_data["traffic"]["receive_port"][3]][1]
            )
        ...
        # send syn packet from Link Port to TAP0 port
        ...
        # send syn packet from TAP1 to TAP2 Port
        ...
        try:
            verify_packets(
                ...
                pkt,
                device_number=0,
                ports=[port_ids[self.config_data["traffic"]["receive_port"][2]][1]],
            )
            log.passed(f" Verification of packets passed, Syn packet received")
        except Exception as err:
            self.result.addFailure(self, sys.exc_info())
        ...
        # send syn+ack packet from Link Port to TAP0 port
        ...
        try:
            ...
            self.result.addFailure(self, sys.exc_info())
            log.failed(
                f" Verification of SynAck packet sent failed with exception {err}"
            )

        # send ack packet from Link port to TAP0 Port
        ...
        # send syn+ack packet from TAP1 to TAP2 Port
        ...
        try:
            verify_packets(
                ...
                pkt,
                device_number=0,
                ports=[port_ids[self.config_data["traffic"]["receive_port"][3]][1]],
            )
        ...
        # send ack packet from TAP1 to TAP2 Port
        log.info("Sending ACK packet: A->B")
        pkt = simple_tcp_packet(
            eth_src=self.config_data["traffic"]["in_pkt_header"]["eth_mac"][2],
            eth_dst=self.config_data["traffic"]["in_pkt_header"]["eth_mac"][3],
        ...
        # Verification of data traffic after connection establishment for TAP0 and Link Port
        ...
        try:
            ...
            log.passed(f" Verification of data packet passed : A->B")
        except Exception as err:
            self.result.addFailure(self, sys.exc_info())
            log.failed(
                f" Verification of data packet sent failed with exception {err}: A->B"
        ...
        try:
            ...
            log.passed(f" Verification of date packet passed: B->A")
        except Exception as err:
            self.result.addFailure(self, sys.exc_info())
            log.failed(
                f" Verification of data packet sent failed with exception {err}: B->A"
        ...
        # Verification of data traffic after connection establishment for TAP1 and TAP2 Port
        log.info(
            "Verification of data traffic after connection establishment for TAP1 and TAP2 Port"
        )
        log.info(
        ...
        # send fin packet A->B from Link to TAP0 port
        ...
        try:
            ...
            log.passed(f" Verification of packets passed, FIN packet received")
        except Exception as err:
            self.result.addFailure(self, sys.exc_info())
            log.failed(f" Verification of FIN packet sent failed with exception {err}")

        # send fin packet A->B from TAP1 to TAP2 port
        ...
        # send fin+ack packet B->A from Link to TAP0 port
        ...
        try:
            ...
            self.result.addFailure(self, sys.exc_info())
            log.failed(
                f" Verification of FIN+ACK packet sent failed with exception {err}"
            )

        # send fin packet B->A from TAP0 port to Link port
        ...
        # send fin+ack packet A->B from Link to TAP0 port
        log.info("Sending ACK packet: A->B")
        pkt = simple_tcp_packet(
            eth_src=self.config_data["traffic"]["in_pkt_header"]["eth_mac"][0],
            eth_dst=self.config_data["traffic"]["in_pkt_header"]["eth_mac"][1],
        ...
        # send fin+ack packet B->A TAP1 and TAP2 Port
        ...
        # send fin packet B->A TAP1 and TAP2 Port
        log.info("Sending FIN packet: B->A")
        pkt = simple_tcp_packet(
            eth_src=self.config_data["traffic"]["in_pkt_header"]["eth_mac"][3],
            eth_dst=self.config_data["traffic"]["in_pkt_header"]["eth_mac"][2],
        ...
        # send fin+ack packet A->B TAP1 and TAP2 Port
        log.info("Sending ACK packet: A->B")
        pkt = simple_tcp_packet(
            eth_src=self.config_data["traffic"]["in_pkt_header"]["eth_mac"][2],
            eth_dst=self.config_data["traffic"]["in_pkt_header"]["eth_mac"][3],
        ...
        # Verification of data traffic after connection closing
        log.info(
            "Verification of data traffic after connection closing for TAP0 and Link Port"
        )
        log.info(
            "----------------------------------------------------------------------------"
        )
        ...
        log.info(
            "Verification of data traffic after connection closing for TAP1 and TAP2 Port"
        )
        log.info(
            "----------------------------------------------------------------------------"
        )
        ...
        # send rst packet TAP0 and Link Port
        ...
        try:
            ...
            log.passed(f" Verification of packets passed, RST packet received")
        except Exception as err:
            self.result.addFailure(self, sys.exc_info())
            log.failed(f" Verification of RST packet sent failed with exception {err}")

        # send rst packet TAP1 and TAP2 Port
