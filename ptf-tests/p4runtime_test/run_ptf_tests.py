...
parser.add_argument("--test-dir", required=True, help="directory containing the tests")
parser.add_argument(
    "--ptf", required=False, default="ptf", help="PTF executable (default: ptf)"
)
parser.add_argument(
    "--arch",
    required=False,
...
parser.add_argument(
    "--target", required=False, default="asic-model", help="Target (asic-model or hw)"
)
parser.add_argument(
    "--port-info",
    required=False,
...
parser.add_argument(
    "--grpc-server", required=False, default="localhost", help="GRPC server IP address"
)
parser.add_argument("--test-params", required=False, default="", help="test params str")
parser.add_argument("--p4c", required=False, default="", help="compiler version")
...
if __name__ == "__main__":
    ...
    if args.port_info is not None and args.port_info != "None":
        ...
        for key in ports.keys():
            new_args += [
                "-i",
                str(ports[key][0]) + "-" + str(ports[key][1]) + "@" + str(key),
            ]
