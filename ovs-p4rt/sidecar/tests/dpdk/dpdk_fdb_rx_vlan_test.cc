...
class DpdkFdbRxVlanTest : public BaseTableTest {
 protected:
  DpdkFdbRxVlanTest() {}

  void SetUp() { SelectTable("l2_fwd_rx_with_tunnel_table"); }
  ...
  void InitFdbInfo() {
    ...
    // Note: 8-bit vlan_id.
    constexpr uint32_t VLAN_ID = 0x42;
  ...
  void CheckDetail() const {
    EXPECT_EQ(detail.table_id, LOG_L2_FWD_RX_WITH_TUNNEL_TABLE);
  }
...
TEST_F(DpdkFdbRxVlanTest, insert_entry) {
  // Arrange
  InitFdbInfo();
  InitAction();
