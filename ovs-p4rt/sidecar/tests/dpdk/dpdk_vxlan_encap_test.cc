...
class DpdkVxlanEncapTest : public BaseTableTest {
 protected:
  DpdkVxlanEncapTest() {}

  void SetUp() { SelectTable("vxlan_encap_mod_table"); }

  //----------------------------
  // Initialization
  //----------------------------

  void InitAction() { SelectAction("vxlan_encap"); }

  void InitTunnelInfo(uint8_t tunnel_type) {
  ...
  void CheckMatches() const {
    constexpr char MOD_DATA_PTR[] = "vendormeta_mod_data_ptr";
    const int MF_MOD_DATA_PTR = GetMatchFieldId(MOD_DATA_PTR);
    ASSERT_NE(MF_MOD_DATA_PTR, -1);

    ASSERT_EQ(table_entry.match_size(), 1);

    const auto& match = table_entry.match()[0];
    ASSERT_EQ(match.field_id(), MF_MOD_DATA_PTR);
...
TEST_F(DpdkVxlanEncapTest, remove_entry) {
  // Arrange
...
TEST_F(DpdkVxlanEncapTest, insert_entry) {
  // Arrange
  InitTunnelInfo(OVS_TUNNEL_VXLAN);
  InitAction();
