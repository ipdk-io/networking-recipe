...
/* Table Sizes */
#ifndef ETHERNET_HOST_SIZE
  #define ETHERNET_HOST_SIZE 65536
#endif

const int _ETHERNET_HOST_SIZE = ETHERNET_HOST_SIZE;
...
control ingress(inout my_ingress_headers_t headers, inout local_metadata_t local_metadata1, in psa_ingress_input_metadata_t standard_metadata, inout psa_ingress_output_metadata_t ostd) {
 ...
 ostd.drop = true;
    ...
    table eth_host_dst {
        key = { headers.ethernet.dst_addr : exact; } 
        actions = {
            send; drop;
        }

        size = _ETHERNET_HOST_SIZE;
    }
