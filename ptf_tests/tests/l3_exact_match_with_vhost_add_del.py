...
"""
DPDK L3 Exact Match (match fields, actions) with vHost add/delete
...
class L3_Exact_Match(BaseTest):
    ...
    def runTest(self):
        ...
        if not result:
            self.result.addFailure(self, sys.exc_info())
            print("FAIL: Ping test failed from VM0")
        
        print("Ping test from VM1 to VM0")
        result = test_utils.vm_to_vm_ping_test(conn2, self.config_data['vm'][1]['remote_ip'])
        if not result:
            self.result.addFailure(self, sys.exc_info())
            print("FAIL: Ping test failed from VM1")

        # delete l3 exact match forwarding rule
        print("Delete Rule 2")
        if not ovs_p4ctl.ovs_p4ctl_del_entry(self.config_data['table'][0]['switch'], self.config_data['table'][0]['name'], self.config_data['table'][0]['del_action'][1]):
            self.result.addFailure(self, sys.exc_info())
            self.fail(f"Failed to delete table entry {self.config_data['table'][0]['del_action'][0]}")

        # ping test between VMs
        print("Ping test from VM0 to VM1 : Drop check")
        result = test_utils.vm_to_vm_ping_drop_test(conn1, self.config_data['vm'][0]['remote_ip'])
        ...
        if not result:
            self.result.addFailure(self, sys.exc_info())
            print("FAIL: Ping drop test failed from VM1")


        # Add Rule 2 back
        print("Add Rule 2 Back")
        if not ovs_p4ctl.ovs_p4ctl_add_entry(self.config_data['table'][0]['switch'],self.config_data['table'][0]['name'], self.config_data['table'][0]['match_action'][1]):
             self.result.addFailure(self, sys.exc_info())
             self.fail(f"Failed to add table entry {self.config_data['table'][0]['match_action'][1]}")
