...
from common.utils.gnmi_cli_utils import gnmi_cli_set_and_verify, gnmi_set_params, ip_set_ipv4,ip_set_dev_up
...
class L3_WCM_Link(BaseTest):
    ...
    def runTest(self):
        ...
        time.sleep(5)
        print("sending packet to TAP0 to check if rule 1 drop")
        pkt = simple_tcp_packet(ip_dst=self.config_data['traffic']['in_pkt_header']['ip_dst'][1])
        
        send_packet(self, port_ids[self.config_data['traffic']['send_port'][1]],
                                            pkt, count=self.config_data['traffic']['num_pkt'])
        try:
        ...
        print("sending packet to check if rule 4 drop")
        pkt = simple_tcp_packet(ip_dst=self.config_data['traffic']['in_pkt_header']['ip_dst'][4],
                                   ip_src=self.config_data['traffic']['in_pkt_header']['ip_src'][0])
        send_packet(self, port_ids[self.config_data['traffic']['send_port'][0]],
                                                    pkt,count=self.config_data['traffic']['num_pkt'])
        try:
        ...
        print("sending packet to check if rule 5 hit")
        pkt = simple_tcp_packet(ip_dst=self.config_data['traffic']['in_pkt_header']['ip_dst'][3],
                                       ip_src=self.config_data['traffic']['in_pkt_header']['ip_src'][0])
        
        send_packet(self, port_ids[self.config_data['traffic']['send_port'][0]], pkt, count=self.config_data['traffic']['num_pkt'])
        try:
            verify_packet(self, pkt, port_ids[self.config_data['traffic']['receive_port'][1]][1])
        ...
        print ("sending packet to check if rule 6 hit")
        pkt = simple_tcp_packet(ip_dst=self.config_data['traffic']['in_pkt_header']['ip_dst'][5],
                                     ip_src=self.config_data['traffic']['in_pkt_header']['ip_src'][0])
       
        send_packet(self, port_ids[self.config_data['traffic']['send_port'][0]], pkt, count=self.config_data['traffic']['num_pkt'])
        try:
            verify_packet(self, pkt, port_ids[self.config_data['traffic']['receive_port'][2]][1])
            print(f"PASS: Verification of packets passed, packet received as per rule 6")
        except Exception as err:
            self.result.addFailure(self, sys.exc_info())
            print(f"FAIL: Verification of packets sent failed with exception {err}")
