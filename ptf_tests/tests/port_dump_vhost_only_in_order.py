...
"""
DPDK Port Dump Test
 1. Verify that TAP, vHost and Link Port can be created separately.
 2. Verify that TAP, vHost and Link Port can be created in mixed way.
 3. Verify that " are same unique postive integer.
 5. Verify that the value "tdi-portin-id" and "tdi-portout-id" for each port are created in 
    incremental way regarless of port ID self.
 6. Verify that port can be created without defaault parameters
 7. Verify that maximum port can be support.
"""
...
class Port_Dump_Test(BaseTest):
    ...
    def runTest(self):
        ...
        print("verify pipe can be set after port configured")
