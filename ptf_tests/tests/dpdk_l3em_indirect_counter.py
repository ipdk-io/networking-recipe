...
class IPDK_L3EM_INDIRECT_COUNTER(BaseTest):
    ...
    def runTest(self):
        ...
        print (f"send intital {self.config_data['counter']['num_ping'][1]} ping to trigger arp etc.")
        if not test_utils.vm_to_vm_ping_test(self.conn_obj_list[0], self.config_data['vm'][0]['remote_ip'],
            ...
            self.result.addFailure(self, sys.exc_info())
            print("FAIL: failed to inital ping")
        ...
        print (f"The counter before sending traffic is {cnt1}")
        
        print("Ping test from VM0 to VM1 to triger counter")
        if not test_utils.vm_to_vm_ping_test(self.conn_obj_list[0], self.config_data['vm'][0]['remote_ip'],
        ...
        for i in range(len(cnt_table_name_and_id)):
            ...
            for j in range(len(self.config_data['counter']['index'])):
                ...
                if cnt_table_name == "ipv4_host_tbl_flow_counter_bytes":
                    ...
                    cnt_change = cnt2[cnt_table_name][j]['bytes'] - cnt1[cnt_table_name][j]['bytes']  
                    #there is a double dumy ping in vm_to_vm_ping_tes
                    expected_bytes = 2*self.config_data['counter']['num_ping'][0]*self.config_data['counter']['default_byte']
                    if cnt_change >= expected_bytes:
                        print (f"PASS: bytes counter is changed by {cnt_change} in flow {flow}")
                    else:
                        self.result.addFailure(self, sys.exc_info())
                        print(f"FAIL: The {cnt_change} is less than expected {expected_bytes} in flow {flow}")
              
                if cnt_table_name == "ipv4_host_tbl_flow_counter_packets":
                    print (f"The curent counter of {cnt_table_name} after sending traffic is {cnt2[cnt_table_name][j]}")
                    cnt_change = cnt2[cnt_table_name][j]['packets'] - cnt1[cnt_table_name][j]['packets']
                    #there is a double dumy ping in vm_to_vm_ping_tes
                    if cnt_change >= 2*self.config_data['counter']['num_ping'][0]:
                        print (f"PASS: packets counter is changed by {cnt_change} in flow {flow}")
                    else:
                        self.result.addFailure(self, sys.exc_info())
                        print(f"FAIL: The {cnt_change} is less than expected {2*self.config_data['counter']['num_ping'][0]}")
