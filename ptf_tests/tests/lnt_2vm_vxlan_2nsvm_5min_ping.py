...
class LNT_2vm_vxlan_2nsvm_5min_ping(BaseTest):
    ...
    def runTest(self):
        ...
        while end_time>= datetime.now():
            print (f"Ping test executed from VM on local host at {datetime.now()}")
            for i in range(len(self.conn_obj_list)):
                for ip in self.config_data['vm'][i]['remote_ip']:
                    print(f"Execute ping test from VM{i} to remote {ip}")
                    if not test_utils.vm_ping_less_than_100_loss(self.conn_obj_list[i], ip):
                        self.result.addFailure(self, sys.exc_info())
