...
class LNT_4VM_2host_add_del_rule(BaseTest):
    ...
    def runTest(self):
        ...
        # ----------------------------------# 
        time.sleep(10)
        print ("Begin to delete and add rules")
        print ("Begin to delete/add vxlan related rules")
        i = 0
        ...
        print ("Begin to delete vlan related rules")
        ...
        for table in self.config_data['table'][:-2]:
            ...
            for i in range(len(table['del_action'])):
                ...
                print (f"Delete rule {table['del_action'][i]}")
                if not ovs_p4ctl.ovs_p4ctl_del_entry(table['switch'], table['name'],table['del_action'][i]):
                    self.result.addFailure(self, sys.exc_info())
